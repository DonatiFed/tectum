...
export default function DebugHUD() {
  ...
  return (
    <div style={{
      ...
      border: '1px solid #ff9500', borderRadius: 6,
      padding: '8px 12px', maxWidth: 480, pointerEvents: 'none',
      boxShadow: '0 4px 16px rgba(0,0,0,0.5)',
    }}>
      <div style={{ color: '#ff9500', fontWeight: 700, marginBottom: 4, letterSpacing: '0.08em' }}>🐞 DEBUG OVERLAY</div>
      {rows.map(([k, v]) => (
        <div key={k}>
          <span style={{ color: '#888' }}>{k.padEnd(13, ' ')}</span>
          <span style={{ color: '#ff9500' }}> {v ?? '—'}</span>
        </div>
      ))}
    </div>
  );
